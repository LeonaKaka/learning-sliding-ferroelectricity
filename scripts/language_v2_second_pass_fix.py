...
# Language V2 is now being repaired page by page.  Keep this relay scoped to
# Module 04 so the workflow cannot silently rewrite later lessons before they
# have been reviewed.
TARGET = base.ROOT / "modules/pinning-creep.html"
base.FILES = [TARGET]

# Exact prose repairs for the visible teaching layer.  base.main() never
# touches .source-text, equations, code, or the paper title.
...
def polish_all_visible_text() -> None:
    ...
    # Header/navigation are intentionally skipped by the generic relay, so fix
    # them explicitly after the reviewed prose pass.
    ...
    # Clean Chinese spacing damage left by the historical mechanical pass.
    for node in list(soup.find_all(string=True)):
        ...
        new = re.sub(r"（\s+", "（", new)
        # Reader-facing editorial vocabulary.
        ...
        new = new.replace("R2 · 跨估计量", "R2 · 跨估计量")
        if new != old:
